...
function iniciar() {
    ...
    if ("serviceWorker" in navigator) {
        navigator.serviceWorker.register("./sw_v25_13.js?v=25.17.0").catch(error => {
...
function construirIndices() {
    ...
    for (let i = 0; i < filasRM.length; i++) {
        ...
        // V25.16.2 — CORRECCIÓN CRÍTICA DE GRUPOS DE REEMPLAZO
        // Cada grupo comienza en una fila con inMue = 1 y termina justo
        // antes del siguiente inMue = 1. La versión anterior tomaba siempre
        // 4 filas desde el padre, lo que mezclaba el siguiente padre con los
        // reemplazos cuando el grupo anterior tenía menos de 3 reemplazos.
        // Eso podía sobrescribir una MGN de muestra con meta incorrecta.
        const siguientePadre = filasRM.findIndex(
            (fila, indice) => indice > i && numero(fila.inMue) === 1
        );
        const finBloque = siguientePadre === -1 ? filasRM.length : siguientePadre;
        const bloque = filasRM.slice(i, finBloque);
...
function construirRegistroSync(vivienda, evento = "RESULTADO") {
    return {
        ...
        gpsLatitud: vivienda?.gpsLatitud ?? "",
        gpsLongitud: vivienda?.gpsLongitud ?? "",
        gpsPrecision: vivienda?.gpsPrecision ?? "",
        gpsFecha: vivienda?.gpsFecha || "",
        gpsEstado: vivienda?.gpsEstado || "",
...
// V25.17.3 — sincronización robusta y cierre con GPS resuelto y evita borrar de la cola
// registros que hayan sido agregados/actualizados mientras un POST estaba en curso.
let crcSincronizacionEnCurso = false;
...
async function sincronizarCola(forzado = false) {
    ...
    if (crcSincronizacionEnCurso) return false;
    crcSincronizacionEnCurso = true;
    ...
    try {
        ...
        const enviados = cola.map(r => JSON.stringify(r));

        // Google Apps Script no expone Access-Control-Allow-Origin para este Web App.
        // Usamos no-cors: el POST se envía sin intentar leer la respuesta.
        ...
        // IMPORTANTE: durante el await puede haber llegado un GPS y reemplazado
        // el snapshot. Solo eliminamos de la cola exactamente lo que acabamos
        // de enviar; nunca borramos un registro nuevo que llegó después.
        const enviadosSet = new Set(enviados);
        const colaActual = obtenerColaSync();
        const colaRestante = colaActual.filter(r => !enviadosSet.has(JSON.stringify(r)));
        guardarColaSync(colaRestante);
        ...
        crcSincronizacionEnCurso = false;

        // Si durante el POST entró un GPS u otro registro, se procesa ahora.
        if (navigator.onLine && obtenerColaSync().length) {
            setTimeout(() => sincronizarCola(), 0);
        }
    }
}
...
async function guardarRecorridoParcial() {
    ...
    await esperarGpsPendienteAntesDeCerrar();
...
async function esperarGpsPendienteAntesDeCerrar() {
    const pendientes = (app.viviendas || []).filter(v =>
        v && v.resultado && v.resultado !== "Pendiente" && v.gpsEstado === "SOLICITANDO"
    );

    if (!pendientes.length) return;

    // El navegador tiene hasta 15 s para resolver la solicitud GPS.
    // Esperamos ese mismo máximo antes de cerrar el snapshot final.
    // Así el último registro no queda enviado con estado SOLICITANDO
    // cuando el GPS todavía está por responder.
    const inicio = Date.now();
    while (Date.now() - inicio < 15500) {
        const quedan = pendientes.filter(v => v.gpsEstado === "SOLICITANDO");
        if (!quedan.length) return;
        await new Promise(resolve => setTimeout(resolve, 250));
    }

    // Si el navegador no respondió dentro del plazo, cerramos el registro
    // con un estado definitivo. Si el callback GPS llega posteriormente,
    // la lógica existente actualizará el registro local y reenviará el snapshot.
    pendientes.forEach(v => {
        if (v.gpsEstado === "SOLICITANDO") {
            v.gpsFecha = new Date().toISOString();
            v.gpsEstado = "TIEMPO_AGOTADO";
        }
    });
    guardarSeleccion();
}

async function finalizarYGuardarRecorrido() {
    const incidenciaAgotada =
        app.estadoRecorrido === "INCIDENCIA_MANZANA" &&
        cicloAgotadoSinMeta();
    ...
    // Antes de construir el snapshot FINAL esperamos a que terminen las
    // capturas GPS pendientes de las viviendas diligenciadas.
    await esperarGpsPendienteAntesDeCerrar();
...
function actualizarGpsEnHistorialCRC(vivienda) {
    for (const manzana of (app.historialManzanas || [])) {
        const encontrada = (manzana.viviendas || []).find(v =>
            String(v?.idVivienda || "") === String(vivienda?.idVivienda || "")
        );
        if (encontrada) {
            encontrada.gpsLatitud = vivienda.gpsLatitud ?? "";
            encontrada.gpsLongitud = vivienda.gpsLongitud ?? "";
            encontrada.gpsPrecision = vivienda.gpsPrecision ?? "";
            encontrada.gpsFecha = vivienda.gpsFecha || "";
            encontrada.gpsEstado = vivienda.gpsEstado || "";
            encontrada.gpsFuente = vivienda.gpsFuente || "";
            return true;
        }
    }
    return false;
...
function actualizarGpsEnRegistroLocalCRC(vivienda) {
    const idVivienda = String(vivienda?.idVivienda || "");
    if (!idVivienda) return null;
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        try {
            const registro = JSON.parse(localStorage.getItem(llave));
            if (!registro?.viviendas?.length) continue;

            const encontrada = registro.viviendas.find(v =>
                String(v?.idVivienda || "") === idVivienda
            );
            ...
            localStorage.setItem(llave, JSON.stringify(registro));
            return { llave, registro };
        } catch (_) {}
    }
    return null;
}

function reconstruirSnapshotGpsLocalCRC(registro) {
    if (!registro) return null;

    const mgn = claveMgn(registro.mgnActual || "");
    const info = mgn ? app.manzanas.get(mgn) : null;
    const grupoId = registro.grupoId || info?.grupoId || (mgn ? "GRUPO_" + mgn : "");
    const estado = String(registro.estadoRecorrido || "").toUpperCase();

    if (estado !== "PARCIAL" && estado !== "FINALIZADA") return null;

    const evento = estado === "FINALIZADA"
        ? "RECORRIDO_FINALIZADO"
        : "RECORRIDO_PARCIAL";

    const padre = registro.grupoId && app.grupos.get(registro.grupoId)?.padre
        ? app.grupos.get(registro.grupoId).padre
        : (info?.grupoId && app.grupos.get(info.grupoId)?.padre
            ? app.grupos.get(info.grupoId).padre
            : mgn);
    const infoPadre = padre ? app.manzanas.get(claveMgn(padre)) : null;
    const meta = infoPadre?.meta || info?.meta || registro.metaManzana || 0;
    const historial = Array.isArray(registro.historialManzanas) ? registro.historialManzanas : [];
    const efectivas = historial.reduce((total, h) =>
        total + (Array.isArray(h?.viviendas) ? h.viviendas.filter(v => v.resultado === "Efectiva").length : 0), 0
    ) + (Array.isArray(registro.viviendas) ? registro.viviendas.filter(v => v.resultado === "Efectiva").length : 0);

    return {
        evento,
        idRecorrido: registro.idRecorrido || "",
        estadoRecorrido: registro.estadoRecorrido || estado,
        sincronizacionActual: "PENDIENTE",
        fecha: registro.ultimaActualizacion || "",
        hora: "",
        fechaTrabajo: registro.fechaTrabajo || "",
        supervisor: registro.supervisor || "",
        encuestador: registro.encuestador || "",
        region: registro.region || "",
        departamento: registro.departamento || "",
        municipio: registro.municipio || "",
        areaGeografica: registro.areaGeografica || "",
        mgn,
        tipoUnidad: info?.tipo || "",
        conteoReal: registro.conteoReal ?? "",
        modoSeleccion: registro.modoSeleccion || "",
        resultadoManzana: registro.resultadoManzana || "",
        motivoManzana: registro.motivoManzana || "",
        detalleMotivoManzana: registro.detalleMotivoManzana || "",
        motivoManzanaCompleto: registro.motivoManzanaCompleto || "",
        vivienda: "",
        idVivienda: "",
        orden: "",
        estado: "",
        ...
        gpsEstado: "",
        grupoId,
        ultimaActualizacion: registro.ultimaActualizacion || new Date().toISOString(),
        viviendas: JSON.parse(JSON.stringify(registro.viviendas || [])),
        historialManzanas: JSON.parse(JSON.stringify(historial)),
        efectivasGrupo: efectivas,
        metaGrupo: meta
    };
}

function capturarGpsVivienda(vivienda) {
    if (!vivienda || !navigator.geolocation) {
        if (vivienda) vivienda.gpsEstado = "NO_DISPONIBLE";
        guardarSeleccion();
    ...
    vivienda.gpsEstado = "SOLICITANDO";
    vivienda.gpsFechaSolicitud = fechaSolicitud;

    navigator.geolocation.getCurrentPosition(
        position => {
            vivienda.gpsLatitud = Number(position.coords.latitude);
            vivienda.gpsLongitud = Number(position.coords.longitude);
            vivienda.gpsPrecision = Number.isFinite(position.coords.accuracy)
                ? Number(position.coords.accuracy.toFixed(2))
                : "";
            vivienda.gpsFecha = new Date(position.timestamp || Date.now()).toISOString();
            vivienda.gpsEstado = "CAPTURADO";
            vivienda.gpsFuente = "GEOLOCATION_API";

            const sigueActual = app.viviendas.includes(vivienda);
            const estaEnHistorial = actualizarGpsEnHistorialCRC(vivienda);

            if (sigueActual) {
                guardarSeleccion();
                reemplazarSnapshotEnCola(construirSnapshotRecorrido("GPS_CAPTURADO"));
            } else if (estaEnHistorial) {
                // La manzana pudo haber pasado a R1/R2/R3 mientras el GPS respondía.
                // Se conserva el dato local; no se genera un evento de servidor por sí solo.
                guardarSeleccion();
            } else {
                // Caso crítico: la manzana pudo haber sido cerrada y limpiada de app.
                // Recuperamos el registro local ya guardado y, si era PARCIAL/FINAL,
                // reenviamos el snapshot con el GPS actualizado.
                const localizado = actualizarGpsEnRegistroLocalCRC(vivienda);
                const snapshot = reconstruirSnapshotGpsLocalCRC(localizado?.registro);
                if (snapshot) reemplazarSnapshotEnCola(snapshot);
            }

            console.info("CRC GPS: capturado", idVivienda, vivienda.gpsLatitud, vivienda.gpsLongitud);
        },
        error => {
            vivienda.gpsFecha = new Date().toISOString();
            const estados = {
                1: "PERMISO_DENEGADO",
                2: "UBICACION_NO_DISPONIBLE",
                3: "TIEMPO_AGOTADO"
            };
            vivienda.gpsEstado = estados[error.code] || "ERROR_GPS";

            const sigueActual = app.viviendas.includes(vivienda);
            const estaEnHistorial = actualizarGpsEnHistorialCRC(vivienda);
            if (sigueActual) {
                guardarSeleccion();
                reemplazarSnapshotEnCola(construirSnapshotRecorrido("GPS_NO_CAPTURADO"));
            } else if (estaEnHistorial) {
                guardarSeleccion();
            } else {
                const localizado = actualizarGpsEnRegistroLocalCRC(vivienda);
            ...
            console.warn("CRC GPS: no fue posible capturar ubicación para", idVivienda, error);
        },
        {
...
function cambiarResultado(indice, valor) {
    ...
    // V25.17.3 — captura GPS puntual al registrar el resultado de cada vivienda.
    // Es asíncrona y no bloquea el flujo de campo; al terminar actualiza el
    // mismo registro local y el snapshot que se sincronizará.
    if (valor !== "Pendiente") capturarGpsVivienda(vivienda);
